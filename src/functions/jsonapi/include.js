...
function makeApiArrayFromObject(data){
    ...
    for (const [id, item] of Object.entries(data)){
        if(item.relationships){
            for (const [field, relationshipItem] of Object.entries(item.relationships)){
...
function include(resp){
            ...
            if(relationship.data){
                if(Array.isArray(relationship.data)){
                    relationship.data.forEach((r) => {
                        ...
                        myResp.data[item.id].relationships[field].data[r.id] = JSON.parse(JSON.stringify(r))
    ...
    resp.included && resp.included.forEach((item) => {
            ...
            mapTargetId[item.type][item.id].forEach(m => {
                ...
                if (entityNest && entityNest[item.id]) {
                    ...
                    if (entityNest[item.id].relationships) {
                        let tempRelationships = JSON.parse(JSON.stringify(entityNest[item.id].relationships))
                        for (const [field, relationship] of Object.entries(tempRelationships)) {
                            ...
                            if (relationship.data) {
                                if (Array.isArray(relationship.data)) {
                                    relationship.data.forEach((r) => {
                                        ...
                                        entityNest[item.id].relationships[field].data[r.id] = JSON.parse(JSON.stringify(r))
                                        ...
                                        mapTargetId[r.type][r.id] = mapTargetId[r.type][r.id].concat(
                                            {
                                                field: m.field + `.${item.id}.relationships.${field}.data`
                                            }
                                        )
                                    })
                                } else {
